...
local Translations = {
	enUS = {
		...
		GEAR_CHECK_LIMITATION = "Gear Check is a surface-level evaluation. It does not consider BiS lists, specific gear builds, encounter requirements, or detailed stat weights. Phase 4 dump shows per-slot OK / REPLACE / BAD from findings.",
		GEAR_CHECK_SCAN = "Scan",
		GEAR_CHECK_HINT = "Target a player (or clear target to scan yourself), then press Scan. Phase 4 dump lists equipment, findings, and verdicts. Run /rw gearcheck test for offline rule fixtures.",
	...
	ruRU = {
		...
		GEAR_CHECK_LIMITATION = "Gear Check — поверхностная оценка. Без BiS, билдов, энкаунтеров и весов статов. Фаза 4: OK / REPLACE / BAD по слотам из findings.",
		GEAR_CHECK_SCAN = "Сканировать",
		GEAR_CHECK_HINT = "Возьмите в цель игрока (или снимите цель, чтобы проверить себя) и нажмите «Сканировать». Фаза 4 — dump с findings и вердиктами. /rw gearcheck test — офлайн-фикстуры правил.",
