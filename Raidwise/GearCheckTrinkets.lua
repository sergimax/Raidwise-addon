...
-- Sphere of Red Dragon's Blood and Darkmoon Card: Death (starter physical DPS).
local TR_STARTER_PHYS = { 37166, 42990 }
...
local TR_NEVERMELTING_ICE_CRYSTAL = { 50259 }
...
local A_PHYS = Ids(P_PHYS, TR_WHISPERING_FANGED_SKULL, TR_DEATHS_VERDICT, TR_NEEDLE, TR_MIRROR, TR_MARK_SUPREMACY, TR_STARTER_PHYS, TR_BANNER, TR_BLOOD_OLD_GOD, TR_MJOLNIR, TR_DARK_MATTER_ULDUAR, TR_PYRITE, TR_GRIM_TOLL, TR_COMET)

local P_RET = Ids(P_PHYS, TR_TINY_ABOM)
local A_RET = Ids(A_PHYS, TR_TINY_ABOM)

local P_HUNTER = Ids(TR_DBW, TR_SHARPENED)
local A_HUNTER = Ids(P_HUNTER, TR_WHISPERING_FANGED_SKULL, TR_DEATHS_VERDICT, TR_NEEDLE, TR_MIRROR, TR_MARK_SUPREMACY, TR_STARTER_PHYS, TR_BANNER)

local P_CASTER = Ids(TR_CHARRED, TR_PHYLACTERY, TR_DISLODGED)
local A_CASTER = Ids(P_CASTER, TR_MURADINS_SPYGLASS, TR_REIGN, TR_SUNDIAL, TR_VOLATILE, TR_JETZE, TR_SHARD_HEART, TR_ILLUSTRATION, TR_FLARE, TR_EYE_BROOD, TR_TALISMAN_RESURGENCE)

local P_HEALER = Ids(TR_GLOWING, TR_ALTHORS, TR_SOLACE, TR_PURIFIED, TR_METEORITE, TR_TALISMAN_RESURGENCE)
local A_HEALER = Ids(P_HEALER, TR_NEVERMELTING_ICE_CRYSTAL, TR_JETZE, TR_SCALE_FATES, TR_SHARD_HEART)
