...
- Implemented gem collector hardening: raw socket enchant IDs remain separate from gem item IDs. Occupied unresolved sockets use `itemId=0` and `state=unresolved`; socket states distinguish confirmed empty, unresolved, and resolved reads. Unknown data keeps the existing bounded inspect retry active, then remains not-checkable if it cannot resolve. Only self tooltip placeholders confirm empty sockets; partial remote reads cannot establish missing gems. Consequently genuine empty remote sockets may remain unverified rather than produce a replacement warning.
- Cache reads merge per socket only for the exact same equipment hyperlink, expire after 10 seconds, and do not retain sockets whose raw enchant field is zero. Changing socket contents replaces the slot's cache entry. Gem reads stay tied to the same link used for the item's stats and identity.
- Meta identity/activation stays not-checkable when required data is unknown. Known gems can still prove activation; complete known colors can still prove inactivity. Exports now include the raw item link, socket enchant IDs, resolution state, and uncertainty/empty-confirmation flags. Existing saved text reports are not repaired; rescan them.
- Offline validation: `npm ci --ignore-scripts`, then `npm run check`. The TypeScript/Node runner uses a pinned Lua 5.1 WebAssembly runtime for the existing rule suite and collector/cache/meta regressions. See `tests/README.md`; the former Python harness has been replaced. No game API extensions or runtime addon dependencies were added.
