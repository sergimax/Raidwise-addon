...
| Settings | `LAYOUT_VERSION = 4` | `PageSettings.lua` | Shell title bar (next to page name) |
...
( ) Composition ( ) History  ( ) Settings ( ) Info
...
| Startup page | Exclusive radio group for left-menu pages; selected page opens on `/raidwise`. Stored in `RaidwiseDB.startupTab` (default `cooldowns`). |
