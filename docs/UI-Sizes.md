...
Below: **Startup page** heading, hint, then an **4-column** grid of menu-page buttons (**28** tall, 8 px gaps); selected page is stored in `RaidwiseDB.startupTab`.

Below: **Unit tooltips** heading, hint, four **24 × 24** checkboxes with labels, then **Preview** with compact + stacked sample blocks (`LAYOUT_VERSION = 3`).
