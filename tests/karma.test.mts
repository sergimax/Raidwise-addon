...
test("global Karma datasets validate, stage, replace atomically, and resolve by character id", async () => {
  ...
  try {
    ...
    for (const module of ["PlayerHistory", "PlayerHistoryStore", "SyncJSON", "KarmaData"]) {
    ...
    lua.doStringSync(`
      ...
      assert(addon:StageGlobalKarmaImport(text,"web"))
      ...
      assert(not addon:StageGlobalKarmaImport(text,"web"))
